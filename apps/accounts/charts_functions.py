...
from django.db.models import Avg, Count, ExpressionWrapper, F, FloatField, Sum, fields
...
def get_user_distribuition_area_pie():
    ...
    user_distribution = User.objects.values(
        "profile__area__name"
    ).annotate(  # Assume que a relação com Profile foi definida como related_name="profile"
        count=Coalesce(Count("profile__area"), 0)
    )
...
def get_user_birthday_by_month_bar():
    ...
    birthday_distribution = (
        Profile.objects.exclude(birthday__isnull=True)  # Exclui funcionários sem data de nascimento
        .values("birthday")
...
def get_average_tenure():
    ...
    tenure_data = Profile.objects.filter(hiring_date__isnull=False).annotate(  # Exclui funcionários sem data de início
        tenure=current_date - F("hiring_date")
    )
