...
class PasswordResetEmailVerifyView(View):
    """View for render password reset from and handle it post data"""
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                print("#" * 100)
                try:
                    emails.send_password_reset(user, user.email, settings.COMPANY_NAME, "", link)
                except Exception as e:  # pylint: disable=broad-exception-caught
                    print("Error sending email for reset password", e)

                del request.session["password_reset_form_data"]
                messages.success(request, "Enviamos um e-mail de recuperação de palavra-passe para si!")
            except Exception as e:  # pylint: disable=broad-exception-caught
